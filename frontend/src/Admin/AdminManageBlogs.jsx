...
function ManageBlogs() {
  ...
  const fetchBlogs = async () => {
    const res = await fetch('https://ss-backend-7arm.onrender.com/api/blogs');
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (isEditing) {
      await fetch(`https://ss-backend-7arm.onrender.com/api/blogs/${editId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(formData)
      });
    } else {
      await fetch('https://ss-backend-7arm.onrender.com/api/blogs', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(formData)
      });
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this blog?')) {
      await fetch(`https://ss-backend-7arm.onrender.com/api/blogs/${id}`, {
        method: 'DELETE'
      });
      fetchBlogs();
    }
